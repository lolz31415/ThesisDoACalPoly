%% Use H5 High Level Commands to create a dataset with array perturbations &
% different DOA source locations
% Original File for training had 2,500 then 25,000 (500 samples 50
% perturbation multipliers) -> For not just single angle need an even
% larger variety...
% h5create, h5write, h5disp, h5read, h5info 
clear all; close all; clc; 
% cd into direct, not relative path 
cd("C:\Users\mathe\Documents\MATLAB\Thesis Codes and Figures\ThesisDatasetsAndNeuralNetwork");

% Validation also used for testing data 
% filename = 'training_data_array_perturbations_angular_range_-90_90_deg.h5';
filename = 'validation_data_array_perturbations_angular_range_-90_90_deg.h5';
...
%% Initialization - automatically overwrites the file
% If script doesn't terminate need to close MATLAB and restart the process:
% HDF5 File closing incorrectly means failure to write dataset

theta_signals = 30; N = 5; d_exp = 0.5; SNR_dB = 20; sig_amp = ones(1,length(theta_signals)); randseed=11; num_snapshots = 1000;
% Even with random theta value, only need initialization once to create the
% BPSK signal, and other necessary data - just need for length of
% theta_signals to be used to determine signal size to be used for creating
% covariance matrix 
...
% Use a joint Validation & Test HDF5 File with Samples 
% samples_per_perturbation_mult = 100e3; % after test on small number
% base_sample_number | perturbation_multiplier_amount
training_data = 1; % samples per multiplier (set below) 
validation_data = 0; % 1/2 training sampler per multiplier (set below) 

% Used 500 for single theta case
% base_sample_number = 500; 
% Create 100,000 total signals 1e5/50 = 2e3
base_sample_number = 2000;
...
% Perturbation Multiplier over span of dataset: 
...
for i = 1:perturbation_multiplier_amount
    ...
    for j = 1:training_sample_number
        % Create still single signal source with random theta_signals 
        % Initialize Parameters unnecessary unless changing signal itself
        % or SNR levels - not angle dependent,
        % "create_perturbed_cov_matrix" uses "theta_signals" to create ASV
        % randn() will potential create sample outside of desired range -
        % use rand() for random theta_signals gerneation for single source
        theta_signals = (theta_max-theta_min).*rand(1) + theta_min; 
...
disp("Time Taken to Write Dataset: "+string(stop_write_time));
